...
Base URL: `https://sendpick.isslab.web.id/api/driver`
...
```
```
4 PUT /status Update status
driver
```
```
5 GET /jobs Get daftar job
orders
```
(^6) **GET /jobs/{jobOrderId
}
Get detail job order
7 POST /jobs/{jobOrderId
}/accept
Terima order
8 POST /jobs/{jobOrderId
}/reject
Tolak order
9 PUT /jobs/{jobOrderId
}/status
Update status job
10 POST /jobs/{jobOrderId
}/pod
Upload Proof of
Delivery
11 POST /scan-qr Scan QR Code
12 POST /gps/bulk Kirim data GPS
batch**
...
```
```
14 GET /history/stats Get statistik drive
```
## 1 ⃣ Login
...
```
```
status string ✅ Available, On Duty,
Off Duty, Tidak
Aktif
```
Response Success (200):
...
```
```
jobOrderId string ID Job Order (contoh:
JO-2024-0001)
```
Response Success (200)
...
"order_type": "Reguler",
"special_instruction": "Barang pecah belah, hati-hati saat
...
```
```
vehicle_id string ❌ ID kendaraan (bisa
diisi nanti)
```
Response Success (200):
...
```
```
404 {"success":^ false,^ "message":^ "Job^
order tidak ditemukan atau sudah
tidak tersedia"}
```
```
422 {"success":^ false,^ "message":^ "Anda^
sudah memiliki 5 order aktif.
Selesaikan order terlebih dahulu."}
```
#### ---
...
```
```
signature file (image) ❌ Tanda tangan
penerima (jpg, jpeg,
png, max 2MB)
```
```
...
```
```
403 {"success": false, "message": "Anda
...
```
```
qr_code_string string ✅ String dari QR code
yang di-scan
```
### Response Success (200)
...
Status Response
```
(^404) **{"success": false, "message": "QR
Code tidak valid atau order tidak
ditemukan"}
403 {"success": false, "message": "Anda
tidak memiliki akses ke order ini"}**
...
```
```
locations array ✅ Array minimal 1
lokasi
```
```
...
```
```
locations[].lng number ✅ Longitude (-180 -
180)
```
```
locations[].sent_at datetime ✅ Waktu pengambilan
GPS
```
```
...
GET /api/driver/history?start_date=2024-12-01&end_date=2024-12-
...
1. Token Storage: Simpan token di secure storage (Keychain iOS /

EncryptedSharedPreferences Android)

2. GPS Tracking: Kirim data GPS dalam batch setiap 30 detik untuk
...
5. Image Compression: Kompres foto POD sebelum upload (max 1MB
